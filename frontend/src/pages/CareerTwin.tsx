...
// "learn cricket", "improve cooking", "master football" must all be rejected.
...
// Career verbs that require subject validation.
// When one of these verbs is found, extract what comes AFTER it and
// check whether that subject is non-career before accepting.
...
// Unambiguous career signals that don't need subject extraction.
...
// Purely structural rejections (gibberish, symbols, repeats)
const STRUCTURAL_REJECT = [
  /^[^a-zA-Z]*$/,   // purely non-alphabetic
  /^(.)\1{4,}$/,    // repeated single char like "aaaaaaa"
];

const isCareerRelevant = (input: string): boolean => {
  const trimmed = input.trim();

  // Reject structural gibberish immediately
  for (const pattern of STRUCTURAL_REJECT) {
    if (pattern.test(trimmed)) return false;
  }

  // If an unambiguous career domain signal is present, accept regardless of verb.
  // e.g. "improve my python skills", "build a docker project" — the domain word
  // alone confirms career intent so no further subject check is needed.
  for (const signal of CAREER_DOMAIN_SIGNALS) {
    if (signal.test(trimmed)) return true;
  }

  // If a career verb is present, extract the subject that follows it and
  // reject if that subject is clearly non-career.
  // e.g. "learn cricket" → subject = "cricket" → NON_CAREER_SUBJECTS match → reject
  // e.g. "learn docker"  → subject = "docker"  → no match → accept
  ...
  if (verbMatch) {
    ...
    // Verb present, subject not blocklisted → treat as career-relevant
    return true;
  }

  // No career verb, no domain signal → reject
  return false;
};
