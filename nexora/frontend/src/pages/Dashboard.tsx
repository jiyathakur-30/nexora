        ...
        fetch('http://localhost:5000/api/analyze', { method: 'POST' })
          ...
          .then(data => {
            ...
            updateMemory(prev => {
              ...
              const newActivities = [...prev.activities];
              if (prev.hasResume) {
                newActivities.push({ id: makeId(), label: 'Resume Profile Analyzed', timestamp: new Date().toISOString() });
                newActivities.push({ id: makeId(), label: 'Skill Gaps Detected', timestamp: new Date().toISOString() });
              }
              newActivities.push({ id: makeId(), label: 'Profile Reanalyzed', timestamp: new Date().toISOString() });
              newActivities.push({ id: makeId(), label: 'Career Twin Generated', timestamp: new Date().toISOString() });
              newActivities.push({ id: makeId(), label: 'Roadmap Updated', timestamp: new Date().toISOString() });

              const mentorGreeting = {
                text: `I've successfully updated your Profile Analysis! Since you enriched your profile sources, I've re-calibrated your Career Twin, readiness scores, and custom weekly goals. Let me know what you'd like to focus on today.`,
                isAi: true,
                timestamp: new Date().toISOString()
              };
          ...
          .catch(err => {
            console.error(err);
            setIsAnalyzing(false);
            alert("Profile analysis failed. Please make sure the backend server is running on port 5000.");
          });
      }
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
            <Card hoverEffect>
              ...
              <div style={{ marginTop: 'var(--space-6)', height: 8, background: 'rgba(61,44,46,0.05)', borderRadius: 4, overflow: 'hidden' }}>
                <motion.div initial={{ width: 0 }} animate={{ width: analysis ? `${analysis.readiness}%` : '0%' }} transition={{ duration: 1, delay: 0.5 }} style={{ height: '100%', background: 'var(--color-primary)' }} />
              </div>
            ...
            <Card hoverEffect>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
                <div style={{ fontSize: '4.5rem', fontWeight: 800, color: 'var(--color-success)', lineHeight: 1 }}>
                  {analysis ? `${Math.min(100, Math.round(analysis.readiness * 1.15))}%` : '--'}
                </div>
              </div>
              <div style={{ marginTop: 'var(--space-6)', height: 8, background: 'rgba(61,44,46,0.05)', borderRadius: 4, overflow: 'hidden' }}>
                <motion.div 
                  initial={{ width: 0 }} 
                  animate={{ width: analysis ? `${Math.min(100, Math.round(analysis.readiness * 1.15))}%` : '0%' }} 
